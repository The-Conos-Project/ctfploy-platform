...
def admin_challenges_page(challenges, toasts=None):
    ...
    return admin_layout(f'''{_toasts(toasts)}<div style="display:flex; align-items:center; justify-content:space-between; gap:12px; flex-wrap:wrap; margin-bottom:18px;"><h1 style="margin:0;">Challenges</h1><button class="secondary" onclick="document.getElementById('import-modal').style.display='flex'" style="display:inline-flex; align-items:center; gap:6px;">{icon("plus")} Import</button></div><section class="card"><h3>Import from URL</h3><form action="/admin/import-url" method="post"><input name="url" placeholder="https://example.com/challenge.tar.gz" required><button style="width: 100%; margin-top: 10px;">Fetch & build</button></form><p class="small-text">A package needs a Dockerfile and ctfploy.json. Docker connection details are detected from EXPOSE.</p></section><div id="import-modal" class="modal" onclick="if(event.target===this)this.style.display='none'"><div class="modal-content" style="width:90%; max-width:480px;"><div style="display:flex; align-items:center; justify-content:space-between; gap:12px; margin-bottom:16px;"><h3 style="margin:0;">Import Challenge</h3><button class="modal-close" onclick="document.getElementById('import-modal').style.display='none'">{icon("circle-x")}</button></div><form action="/admin/import-url" method="post"><input name="url" placeholder="https://example.com/challenge.tar.gz" required style="margin-bottom:12px;"><button type="submit">Fetch & build</button></form></div></div><section class="card"><h2>All challenges</h2><ul class="list">{rows or '<li>No challenges imported yet.</li>'}</ul></section>''', active='boxes')
...
def admin_classes_page(classes, challenges, users, toasts=None):
    ...
    for classroom in classes:
        ...
        rows += f'''<li><div class="row"><div><strong>{escape(classroom['name'])}</strong><div class="small-text">{members} student(s) · {assignments} assignment(s) · Join code: <code>{escape(classroom['join_code'])}</code></div></div><div style="display:flex; gap:8px;"><a href="/admin/classes/{classroom['id']}"><button class="secondary">Open</button></a><form method="post" action="/admin/classes/delete/{classroom['id']}" onsubmit="return confirm('Delete this class?')"><button class="secondary" style="color:#ffb3c1; border-color:#5e2230;">Delete</button></form></div></div></li>'''
    return admin_layout(f'''{_toasts(toasts)}<div style="display:flex; align-items:center; justify-content:space-between; gap:12px; flex-wrap:wrap; margin-bottom:18px;"><h1 style="margin:0;">Classes</h1><button class="secondary" onclick="document.getElementById('create-modal').style.display='flex'" style="display:inline-flex; align-items:center; gap:6px;">{icon("plus")} New Class</button></div><p class="muted">Create a class, share its join code, then open it to assign challenges.</p><section class="card"><ul class="list">{rows or '<li>No classes yet.</li>'}</ul></section><div id="create-modal" class="modal" onclick="if(event.target===this)this.style.display='none'"><div class="modal-content" style="width:90%; max-width:480px;"><div style="display:flex; align-items:center; justify-content:space-between; gap:12px; margin-bottom:16px;"><h3 style="margin:0;">Create Class</h3><button class="modal-close" onclick="document.getElementById('create-modal').style.display='none'">{icon("circle-x")}</button></div><form method="post" action="/admin/classes/create"><input name="name" placeholder="Linux Foundations — Group A" required style="margin-bottom:12px;"><button type="submit">Create class</button></form></div></div>''', active='users')
...
def admin_class_detail_page(classroom, challenges, users, toasts=None):
    ...
    return admin_layout(f'''{_toasts(toasts)}<a href="/admin/classes" class="small-text">{icon("arrow-left")} All classes</a><h1>{escape(classroom['name'])}</h1><p class="muted">Join code: <code>{escape(classroom['join_code'])}</code></p><div class="grid"><section class="card"><h3>Assign a challenge</h3><form method="post" action="/admin/classes/assign"><input type="hidden" name="class_id" value="{classroom['id']}"><select name="challenge_id">{options}</select><button style="width: 100%; margin-top: 10px;">Assign challenge</button></form><h3>Assigned challenges</h3><ul class="list">{assigned_html}</ul></section><section class="card"><h3>Students</h3><ul class="list">{members}</ul></section></div><form method="post" action="/admin/classes/delete/{classroom['id']}" onsubmit="return confirm('Delete this class?')" style="margin-top:12px;"><button class="secondary" style="color:#ffb3c1; border-color:#5e2230;">Delete class</button></form>''', active='users')


def admin_update_page(toasts=None):
    return admin_layout(f'''{_toasts(toasts)}<section class="card"><h2>Update platform</h2><p>Pull the latest image and restart the platform container.</p><form method="post"><button>Update now</button></form></section>''', active='settings')
