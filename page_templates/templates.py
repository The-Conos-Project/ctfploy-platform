...
def admin_challenges_page(challenges, toasts=None):
    ...
    for ch in challenges:
        ...
        update_btn = f'<form method="post" action="/admin/update_challenge/{ch["id"]}" onsubmit="return confirm(\'Rebuild this challenge from its source URL?\')" style="display:inline;"><button type="submit" class="secondary" style="font-family:inherit;">Update</button></form>' if source_url else ''
        rows += f'''<li><div class="row"><div><strong>{escape(ch['display_name'])}</strong><div class="small-text">{escape(ch.get('description', ''))}</div><span class="status-badge status-{status}">{label}</span></div><div style="display:flex; gap:8px; flex-wrap:wrap;">{update_btn}<a href="/admin/build_log/{ch['id']}" class="small-text">Build logs</a> · <a href="/admin/delete_challenge/{ch['id']}" class="small-text" style="color:#ffb3c1;font-weight:600;">Delete</a></div></div></li>'''
